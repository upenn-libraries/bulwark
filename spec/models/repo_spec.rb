...
RSpec.describe Repo, type: :model do
  ...
  let(:repo_instance) { FactoryBot.create(:repo) }

  it "is invalid without a title" do
    expect(FactoryBot.build(:repo, :title => nil)).not_to be_valid
  end
  it "is invalid without a metadata subdirectory" do
    expect(FactoryBot.build(:repo, :metadata_subdirectory => nil)).not_to be_valid
  end
  it "is invalid without an assets subdirectory" do
    expect(FactoryBot.build(:repo, :assets_subdirectory => nil)).not_to be_valid
  end
  it "is invalid without a metadata filename" do
    expect(FactoryBot.build(:repo, :metadata_filename => nil)).not_to be_valid
  end
