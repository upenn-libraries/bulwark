...
class MetadataBuilder < ActiveRecord::Base
  ...
  def file_checks_previews(working_path)
    ...
    if self.metadata_source.where(:source_type => MetadataSource.structural_types).empty?
      Dir.glob("#{working_path}/#{self.repo.assets_subdirectory}/*.{#{self.repo.file_extensions.join(",")}}").each do |file_path|
        ...
        generate_preview(valid_file,"#{working_path}/#{self.repo.derivatives_subdirectory}") unless validation_state.present?
  ...
  def generate_preview(file, derivatives_directory)
    preview = Utils::Derivatives::Preview.generate_copy(file, derivatives_directory)
    thumbnail = Utils::Derivatives::PreviewThumbnail.generate_copy(file, derivatives_directory)
